...
- `QtProject/tests`: `test_repo_*` targets are self-contained and use only checked-in fixtures. `test_local_*` targets use the optional `/Dev` corpus, while `test_large_video_corpus` is the separately mounted 100GB corpus. `test_repo_auto_delete` covers focused end-to-end cleanup; `test_repo_video_extraction_regression` snapshots the Nice videos; and `test_repo_video_matching` validates the tracked matching manifest.
