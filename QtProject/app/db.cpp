...
{
    if (!prefs.cacheFilePathName().isEmpty() && initDbAndCache(prefs))
        return true;
...
bool Db::emptyAllDb(const Prefs prefs)
{
    ...
    { // isolate queries, so that when we removeDatabase, it doesn't warn of possible problems
        QSqlDatabase db = QSqlDatabase::addDatabase(QStringLiteral("QSQLITE"), connexionName);
...
bool Db::initDbAndCache(const Prefs& prefs)
{
    ...
    const QString uniqueConnexionName =
        QUuid::createUuid().toString(); // each instance of Db must connect separately, uniquely
    {
        ...
        if (!db.open()) {
            Message::Get()->add(QString("Failed to open db file with driver %1\n    File: %2 - error: %3")
                                    .arg(db.driverName(), prefs.cacheFilePathName(), db.lastError().text()));
            return false;
        }
...
QString Db::getUserSelectedCacheNamePath(const Prefs& prefs)
{
    ...
    QString dbfilename =
        QFileDialog::getSaveFileName(prefs._mainwPtr, "Save/Load Cache",
                                     QStandardPaths::writableLocation(QStandardPaths::HomeLocation), "Cache (*.db)");
    return dbfilename;
}
...
void Db::writeMetadata(const Video& video) const
{
    ...
    QSqlError error = query.lastError();
    if (error.isValid()) {
        qDebug() << "Error with writeMetadata query for video=" << video._filePathName
