...
void SimplifiedTestHelpers::loadMetadataFromFile(const QString& filePath, VideoParam& param)
{
    ...
    QVERIFY2(data.contains("videoFilename"),
             QString("Video filename not found in metadata file: %1").arg(filePath).toUtf8());
    ...
    QVERIFY2(param.videoInfo.exists(),
             QString("Video file not found: %1").arg(param.videoInfo.absoluteFilePath()).toUtf8());
    ...
    QVERIFY2(param.thumbnailInfo.exists(),
             QString("Thumbnail file not found: %1").arg(param.thumbnailInfo.absoluteFilePath()).toUtf8());
...
double SimplifiedTestHelpers::compareThumbnails(const QByteArray& thumb1, const QByteArray& thumb2)
{
    ...
    if (gray1.size() != gray2.size()) {
        qWarning() << "Thumbnail dimensions don't match:" << gray1.cols << "x" << gray1.rows << "vs" << gray2.cols
                   << "x" << gray2.rows;
        return 0.0;
    }
...
Mat SimplifiedTestHelpers::thumbnailToGrayMat(const QByteArray& thumbnail)
{
    ...
    Mat mat(grayImage.height(), grayImage.width(), CV_8UC1, const_cast<uchar*>(grayImage.bits()),
