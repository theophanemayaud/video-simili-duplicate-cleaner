...
- Existing cache records are reused without a generation marker or forced invalidation. Restored JPEG captures go through the same new analysis pipeline. Fresh extraction gets the improved presentation-metadata normalization; an already cached metadata-rotated video may retain its previous dimensions/orientation until a fresh/no-cache scan or explicit cache removal, which is an accepted small tradeoff for keeping cache handling simple.
